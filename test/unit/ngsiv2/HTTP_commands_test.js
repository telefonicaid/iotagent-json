...
describe('HTTP: Commands with extra headers', function () {
    beforeEach(function (done) {
        const provisionOptions = {
            url: 'http://localhost:' + config.iota.server.port + '/iot/devices',
            method: 'POST',
            json: utils.readExampleFile('./test/deviceProvisioning/provisionCommandHTTP3.json'),
            headers: {
                'fiware-service': 'smartgondor',
                'fiware-servicepath': '/gardens'
            }
    ...
    describe('When a command arrive to the Agent for a device with the HTTP protocol', function () {
        ...
        beforeEach(function () {
            ...
            mockedClientServer = nock('http://localhost:9876')
                .matchHeader('MyCustomHeader', 'MQTT_2AnMQTTDevice')
                .post('/command', function (body) {
                    return body || body === 'MQTT_2AnMQTTDevice';
                })
                .reply(200, '{"PING":{"data":"22"}}');
...
describe('HTTP: Commands with extra headers from groups', function () {
    beforeEach(function (done) {
        config.logLevel = 'INFO';
    ...
    describe('When a POST measure arrives for an unprovisioned device in a command group', function () {
        ...
        beforeEach(function (done) {
            ...
            request(groupCreation2, function (error, response, body) {
                done();
            });
        });
        ...
        describe('When a command arrive to the Agent for a device with the HTTP protocol', function () {
            ...
            beforeEach(function () {
                ...
                contextBrokerMock
                    ...
                    .reply(204);
                mockedClientServer = nock('http://localhost:9876')
                    .matchHeader('MyCustomHeader', 'JSON_UNPROVISIONEDTheLightType')
                    .post('/command', function (body) {
                        return body.cmd1 || body.cmd1.data || body.cmd1.data === 22;
                    })
                    .reply(200, '{"cmd1":{"data":"22"}}');
