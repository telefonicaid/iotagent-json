...
of devices with a new API Key is provisioned first). Please, make sure you fulfil all the requirements from the next
...
work as well (previous versions could also work, but also may not, so we encourage you to use versions above).
...
* In the first section, a set of topics is defined for the default APIKey (`1234` in this case). These topics are marked
