...
function generateCommandExecution(apiKey, device, attribute) {
    ...
    if (device.transport === 'AMQP') {
        ...
        command.payloadType = command && command.payloadType ? command.payloadType : 'binaryfromjson';
    }
    const serialized = serializedPayloadCommand(payload, command);
    const contentType = command.contentType ? command.contentType : 'application/json';
    config
        .getLogger()
        .debug(
            context,
