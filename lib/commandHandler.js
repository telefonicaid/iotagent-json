...
function generateCommandExecution(apiKey, device, attribute) {
    ...
    if (command && command.expression) {
        ...
        let ctxt = device.staticAttributes.concat(attribute);
        // TBD context:
        // ctxt += entity attrs in a time ?
        payload[attribute.name] = parser.applyExpression(command.expression, ctxt, device);
    } else {
