...
function start(newConfig, callback) {
    ...
    // Propagate by copy mqtt config to iotanode-lib
    config.getConfig().iota.mqtt = {
    ...
    iotAgentLib.activate(config.getConfig().iota, function (error) {
        if (error) {
            ...
            function setReady(err) {
                ...
                config.getLogger().info(context, 'IoT Agent READY');
                iotAgentLib.statsRegistry.set('ready', 1, callback);
            }
            transportSelector.startTransportBindings(newConfig, setReady);
        }
    });
}
