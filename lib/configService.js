...
function processEnvironmentVariables() {
    ...
    if (process.env.IOTA_JEXL_EXTRA_PATH) {
        const p = path.resolve(process.env.IOTA_JEXL_EXTRA_PATH);
        fileExists(p);
        const extraConfig = require(p);
        config.jexlTransformations = extraConfig.jexlTransformations;
    }
}
