...
function parseBattery(payload) {
    ...
    config
        .getLogger()
        .debug(context, 'Parsing Battery module: %s', JSON.stringify(fields, null, 4));
    ...
    if (result.length !== 0) {
        ...
        config
            .getLogger()
            .error(context, 'TTHINGS-001: Too few fields parsing Battery module: %s', rawValue);
        return [];
    }
}
...
function parseGSM(payload) {
    ...
    if (fields.length >= 7) {
        ...
        config
            .getLogger()
            .error(context, 'TTHINGS-002: Too few fields parsing GSM module: %s', rawValue);
        return [];
    }
}
...
function parseCell(payload) {
    ...
    if (fields.length === 4) {
        ...
        config
            .getLogger()
            .error(context, 'TTHINGS-003: Too few fields parsing C1 module: %s', rawValue);
        return [];
    }
}
