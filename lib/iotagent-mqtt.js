...
function manageConfigurationRequest(apiKey, deviceId, device, message) {
    ...
    function sendConfigurationToDevice(results, callback) {
        ...
        if (results.contextResponses && results.contextResponses[0] &&
            ...
            mqttClient.publish('/1234/MQTT_2/configuration/commands', JSON.stringify(configurations), null, callback);
        } else {
            callback();
        }
    }
 ...
 * @return {Array}                  List of strings names of the topics associated to the selected device.
 */
function generateDeviceTopics(device) {
    var topics = [];
    ...
    topics.push('/' + config.mqtt.defaultKey + '/' + device.id + '/configuration/commands');
...
function deviceProvisioningHandler(device, callback) {
    ...
    if (mqttClient) {
        topics = generateDeviceTopics(device);
