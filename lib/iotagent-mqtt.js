...
var iotAgentLib = require('iotagent-node-lib'),
    ...
    commandHandler = require('./commandHandlers'),
...
function mqttMessageHandler(topic, message) {
    ...
    function processDeviceMeasure(error, device) {
        if (error) {
            ...
            if (topicInformation[3] === 'configuration' && topicInformation[4] === 'commands' && parsedMessage) {
                ...
                updateCommand(apiKey, deviceId, device, parsedMessage);
    ...
    if (apiKey === config.mqtt.defaultKey) {
...
function configurationHandler(configuration, callback) {
    if (configuration.resource && config.iota.iotManager && config.iota.defaultResource &&
        configuration.resource !== config.iota.defaultResource) {
