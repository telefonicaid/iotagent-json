...
function parseMessage(message) {
    ...
    try {
        ...
        logger.debug('Parse error treating message: %s', stringMessage);
        parsedMessage = null;
    }
...
function recreateSubscriptions(callback) {
    ...
    iotAgentLib.getDevicesByAttribute('protocol', 'GENERIC_PROTO', undefined, undefined, function(error, devices) {
        if (error) {
            logger.info(context, 'No devices found for subscription.');
            callback();
        } else {
