    ...
    function sendConfigurationToDevice(results, callback) {
        ...
        if (results.contextResponses && results.contextResponses[0] &&
            ...
            for (var i = 0; i < results.contextResponses[0].contextElement.attributes.length; i++) {
                configurations.push({
                    name: results.contextResponses[0].contextElement.attributes[i].name,
                    value: results.contextResponses[0].contextElement.attributes[i].value
                });
            }
 ...
 */
function mqttMessageHandler(topic, message, packet) {
