...
function manageConfigurationRequest(apiKey, deviceId, device, objMessage) {
    ...
    function sendConfigurationToDevice(results, callback) {
        ...
        if (results.contextResponses && results.contextResponses[0] &&
            ...
            for (var i = 0; i < results.contextResponses[0].contextElement.attributes.length; i++) {
                configurations[results.contextResponses[0].contextElement.attributes[i].name] =
                    results.contextResponses[0].contextElement.attributes[i].value;
            }

            configurations.dt = dateFormat(now, 'yyyymmdd\'T\'hhMMss\'Z\'');
...
function configurationHandler(configuration, callback) {
    if (configuration.resource && configuration.resource !== '') {
