...
function parseMessage(message) {
    ...
    if (!parsedMessage) {
        logger.error(context, 'MEASURES-003: Impossible to handle malformed message: %s', message);
    }
...
function recreateSubscriptions(callback) {
    ...
    function subscribeToTopics(topics, callback) {
        ...
        mqttClient.subscribe(topics, null, function(error) {
            if (error) {
                logger.error('GLOBAL-001: Error subscribing to topics: %s', error);
                callback(error);
            } else {
...
function manageConfigurationRequest(apiKey, deviceId, device, objMessage) {
    iotaUtils.manageConfiguration(apiKey, deviceId, device, objMessage, sendConfigurationToDevice, function() {
        logger.debug('Configuration request finished for APIKey [%s] and Device [%s]', apiKey, deviceId);
    });
}
...
function mqttMessageHandler(topic, message) {
    ...
    function processDeviceMeasure(error, device) {
        if (error) {
            logger.error(context, 'MEASURES-004: Device not found for topic [%s]', topic);
        } else {
