...
function singleMeasure(apiKey, deviceId, attribute, device, message) {
    ...
    config.getLogger().debug('Processing single measure for device [%s] with apiKey [%s]', deviceId, apiKey);
...
function multipleMeasures(apiKey, deviceId, device, messageObj) {
    ...
    config.getLogger().debug('Processing multiple measures for device [%s] with apiKey [%s]', deviceId, apiKey);
...
function parseMessage(message) {
    ...
    try {
        ...
        config.getLogger().debug('Parse error treating message [%s]: %j', stringMessage, e);
        parsedMessage = null;
    }
...
function recreateSubscriptions(callback) {
    ...
    function subscribeToTopics(topics, callback) {
        ...
        mqttClient.subscribe(topics, null, function(error) {
            if (error) {
                config.getLogger().error('GLOBAL-001: Error subscribing to topics: %s', error);
                callback(error);
            } else {
...
function sendConfigurationToDevice(apiKey, deviceId, results, callback) {
    ...
    config.getLogger().debug('Sending requested configuration to the device:\n %j', configurations);
...
function manageConfigurationRequest(apiKey, deviceId, device, objMessage) {
    iotaUtils.manageConfiguration(apiKey, deviceId, device, objMessage, sendConfigurationToDevice, function() {
        config.getLogger().debug('Configuration request finished for APIKey [%s] and Device [%s]', apiKey, deviceId);
    });
}
