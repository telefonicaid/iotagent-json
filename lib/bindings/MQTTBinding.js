    ...
    function subscribeToTopics(topics, callback) {
        ...
        mqttClient.subscribe(topics, null, function(error) {
            if (error) {
                ...
                config
                    .getLogger()
                    .error(context, 'GLOBAL-001: Error subscribing to topics: %s', error);
                callback(error);
            } else {
...
function sendConfigurationToDevice(apiKey, deviceId, results, callback) {
    ...
    config
        .getLogger()
        .debug(context, 'Sending requested configuration to the device:\n %j', configurations);
...
function stop(callback) {
    async.series([unsubscribeAll, mqttClient.end.bind(mqttClient, true)], callback);
}
