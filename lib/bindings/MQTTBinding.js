...
function start(callback) {
    ...
    const options = {
        ...
        clientId: mqttConfig.clientId ? mqttConfig.clientId : 'iotajson_' + Math.random().toString(16).substr(2, 8),
    ...
    config
        .getLogger()
        .debug(context, 'Starting MQTT binding with options %j retries %s retryTIme %s', options, retries, retryTime);
    ...
    function createConnection(callback) {
        ...
        options.clientId = mqttConfig.clientId
            ? mqttConfig.clientId
            : 'iotajson_' + Math.random().toString(16).substr(2, 8);
