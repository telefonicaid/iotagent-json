...
function executeCommand(apiKey, device, serializedPayload, callback) {
    const options = {};
    options.qos =
        serializedPayload.mqtt && serializedPayload.mqtt.qos
            ? serializedPayload.mqtt.qos
            : config.getConfig().mqtt.qos
            ? parseInt(config.getConfig().mqtt.qos)
            : 0;
    options.retain =
        serializedPayload.mqtt && serializedPayload.mqtt.retain
            ? serializedPayload.mqtt.retain
