...
function handleIncomingMeasure(req, res, next) {
    ...
    iotaUtils.retrieveDevice(req.deviceId, req.apiKey, group.transport || transport, processDeviceMeasure);
}
...
function sendConfigurationToDevice(apiKey, group, deviceId, results, callback) {
    ...
    iotaUtils.retrieveDevice(deviceId, apiKey, group.transport || transport, function (error, device) {
        if (error) {
            callback(error);
