    ...
    got.post(device.endpoint, options)
        .then((response) => {
            ...
            if (response.statusCode !== 200 && response.statusCode !== 201) {
                return callback(new errors.HTTPCommandResponseError(response ? response.statusCode : 400, ''));
            } else if (apiKey) {
                let parsedBody;
                try {
                    ...
                    return callback(new errors.HTTPCommandResponseError(400, e));
                }
            } else {
                return callback();
            }
        ...
        .catch((error) => {
            return callback(new errors.HTTPCommandResponseError(400, error));
        });
}
...
function sendConfigurationToDevice(apiKey, deviceId, results, callback) {
    function sendRequest(device, results, innerCallback) {
        ...
        got.post(device.endpoint + constants.HTTP_CONFIGURATION_PATH, resultRequest)
            .then((response) => {
                if (response.statusCode !== 200) {
                    return innerCallback(new errors.DeviceEndpointError(response.statusCode, response.body));
                } else {
