...
function handleIncomingMeasure(req, res, next) {
    ...
    config.getLogger().debug('Processing multiple HTTP measures for device [%s] with apiKey [%s]',
...
function sendPushNotifications(device, values, callback) {
    async.series(values.map(commandHandler.generateCommandExecution.bind(null, null, device)), function(error) {
        callback(error);
    });
}
