...
function executeCommand(apiKey, device, cmdName, serializedPayload, contentType, callback) {
    ...
    if (device.endpoint.expression) {
        ...
        attrList = device.staticAttributes ? attrList.concat(device.staticAttributes) : [];
        let ctxt = parser.extractContext(attrList, device);
        // expression result will be the full command payload
        let endpointRes = parser.applyExpression(device.endpoint.expression, ctxt, device);
        options.url.device.endpoint = endpointRes ? endpointRes : device.endpoint.expression;
    }
