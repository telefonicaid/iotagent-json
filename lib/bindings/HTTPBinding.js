...
function sendPushNotifications(device, group, values, callback) {
    const executions = _.flatten(values.map(commandHandler.generateCommandExecution.bind(null, null, device, group)));
...
function notificationHandler(device, values, callback) {
    config.getLogger().debug(context, 'values for command [%j] ', values);
    ...
    function invokeWithConfiguration(apiKey, callback) {
        ...
        iotAgentLib.getConfigurationSilently(config.getConfig().iota.defaultResource || '', apiKey, function (
            ...
            const cmdObj = values.find((obj) => {
                return obj.name === 'cmd';
            });
            const paramsObj = values.find((obj) => {
            ...
            var cmdValues = [
                {
                    ...
                    type: 'command'
                    // TBD copy all others fields from notification command ?
                    //status.value
                    //info.value
                    // onDelivered onOk onError onInfo cmdExecution dateExpiration
                }
            ];
