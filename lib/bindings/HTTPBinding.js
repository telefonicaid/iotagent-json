        ...
        if (error || (response.statusCode !== 200 && response.statusCode !== 201)) {
            ...
            if (apiKey) {
                ...
                try {
                    ...
                    process.nextTick(
                        commandHandler.updateCommand.bind(
                            null,
                            apiKey,
                            device.id,
                            device,
...
function handleIncomingMeasure(req, res, next) {
    ...
    config
        ...
        .debug(
            context,
            'Processing multiple HTTP measures for device [%s] with apiKey [%s]',
            req.deviceId,
            req.apiKey
        );
...
function returnCommands(req, res, next) {
    ...
    if (req.query && req.query.getCmd === '1') {
        iotAgentLib.commandQueue(req.device.service, req.device.subservice, req.deviceId, function(
            error,
            list
        ) {
...
function start(callback) {
    ...
    config
        .getLogger()
        .info(context, 'HTTP Binding listening on port [%s]', config.getConfig().http.port);
...
function sendPushNotifications(device, values, callback) {
    var executions = _.flatten(
        values.map(commandHandler.generateCommandExecution.bind(null, null, device))
    );
