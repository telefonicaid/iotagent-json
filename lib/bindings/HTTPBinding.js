...
const xml = promisify(
    bodyParser.xml({
        ...
        // but we make them explicit for clarity.
        inflate: true,
        limit: '100kb',
        xmlParseOptions: {
            attrKey: '$',
            ...
            trim: true, // non-default
            normalizeTags: false,
            normalize: false,
            explicitRoot: true,
            emptyTag: '',
            explicitArray: false, // non-default
            ignoreAttrs: false,
            mergeAttrs: false,
            xmlns: false,
...
function checkContentType(...mimeTypes) {
    return function (req, res, next) {
        let error;
        if (req.method === 'POST' && !_.some(mimeTypes, req.s)) {
            error = new errors.UnsupportedType(mimeTypes.join(', '));
        }
        next(error);
    };
}
