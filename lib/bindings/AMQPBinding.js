...
function start(callback) {
    ...
    function createListener(queueObj, callback) {
        ...
        amqpChannel.bindQueue(queueObj, exchange, '.*.*.attrs.#');
        amqpChannel.consume(queueObj, queueListener, {noAck: true});
        config.getLogger().debug(context, 'subscribed to updates queue');

        amqpChannel.bindQueue(queueObj + '_commands', exchange, '.*.*.cmdexe');
        amqpChannel.consume(queueObj + '_commands', queueListener, {noAck: true});
