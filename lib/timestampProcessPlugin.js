...
function updatePlugin(entity, entityType, callback) {
    ...
    if (
        entity.contextElements &&
        entity.contextElements[0] &&
        entity.contextElements[0].attributes
    ) {
        ...
        if (timestamp) {
            entity.contextElements[0].attributes = entity.contextElements[0].attributes.map(
                insertMetadata
            );
        }
        ...
        config
            .getLogger()
            .error(context, 'MEASURES-001: Bad payload received while processing timestamps');
