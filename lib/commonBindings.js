...
function parseMessage(message) {
    ...
    try {
        ...
        config
            .getLogger()
            .debug(context, 'Parse error treating message [%s]: %j', stringMessage, e);
        parsedMessage = null;
    }
    ...
    if (!parsedMessage) {
        config
            .getLogger()
            .error(context, 'MEASURES-003: Impossible to handle malformed message: %s', message);
    }
...
function messageHandler(topic, message, protocol) {
    ...
    function processDeviceMeasure(error, device) {
        if (error) {
            config
                .getLogger()
                .error(context, 'MEASURES-004: Device not found for topic [%s]', topic);
        } else {
            if (
                topicInformation[3] === 'configuration' &&
                topicInformation[4] === 'commands' &&
                parsedMessage
            ) {
